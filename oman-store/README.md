...
## 9.2 ملاحظة إصلاح: Build timeout على Render

إذا واجهت خطأ `Static page generation for /register is still timing out` (أو أي صفحة أخرى)، فالسبب أن Next.js حاول بناء الصفحة كـ Static عند البناء رغم أنها تعتمد على قاعدة البيانات/الجلسة وقت الطلب. كل الصفحات التي تحتاج ذلك في هذا المشروع تحمل السطر التالي أعلاها الآن:

```ts
export const dynamic = "force-dynamic";
```

إن أضفت صفحة جديدة تستخدم `db` أو `auth()`، أضف نفس السطر أعلاها لتفادي نفس المشكلة.

تذكير: `.gitignore` يستثني `node_modules/` و`.env` و`.next/` تلقائيًا — تأكد أنك لا ترفع `.env` نفسه أبدًا لـ GitHub، فقط `.env.example`.
